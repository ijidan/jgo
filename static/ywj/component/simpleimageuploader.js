...
define('ywj/simpleimageuploader', function(require){
	...
	var su = function(input, config){
		...
		PRI.button = $('<label class="'+this.config.CLS_UPLOAD_BUTTON+'" for="'+this.id+'">upload image</label>');
		PRI.image_container = $('<span class="'+this.config.CLS_UPLOAD_IMAGE_THUMB+'"><a href="" target="_blank"><img alt=""/></a><span class="'+this.config.CLS_IMAGE_DELETE+'">delete</span></span>');
		...
		file_input.on('change', function(){
			...
			xhr.onload = function(){
				...
				if(xhr.status === 200){
					...
					_this.onError('error，try it later(3)');
				}
			};
	...
	su.prototype.updateDomState = function(to, message){
		...
		switch(to){
			case this.config.CLS_UPLOADING:
				PRIVATES[this.id].message.html(message || 'uploading···');
				break;

			case this.config.CLS_UPLOAD_NORMAL:
				PRIVATES[this.id].button.html('upload image');
				break;
	...
	su.prototype.onResponse = function(rsp_str){
		...
		if(rsp.code == '0'){
			...
			this.onError(rsp.message || 'error,try it later');
		}
	};
	...
	su.prototype.onUploading = function(percent){
		...
		PRIVATES[this.id].message.html('uploading···');
	...
	su.prototype.onSuccess = function(message, data){
		...
		PRIVATES[this.id].button.html('re-upload');
	...
	su.prototype.onError = function(message){
		...
		PRIVATES[this.id].message.html(message || 'fail，try it later');
	};
